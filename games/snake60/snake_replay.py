...
def normalize_entry(item, index=0):
    ...
    return {
        ...
        "name": sanitize_text(item.get("name", "ANON"), PLAYER_NAME_MAX_LEN, uppercase=True)
        if isinstance(item, dict)
        else "ANON",
        "score": clamp_int(item.get("score", 0) if isinstance(item, dict) else 0, 0, 999999),
        "message": sanitize_text(item.get("message", "") if isinstance(item, dict) else "", PLAYER_MESSAGE_MAX_LEN),
