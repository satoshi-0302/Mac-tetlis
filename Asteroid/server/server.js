...
const server = createServer(async (request, response) => {
  ...
  if (url.pathname === '/api/leaderboard' && method === 'GET') {
    sendJson(response, 200, buildLeaderboardPayload());
    return;
  }
  ...
  if (url.pathname === '/api/replay' && method === 'GET') {
    ...
    if (kind === 'human') {
      ...
      if (!Number.isInteger(replayId) || replayId <= 0) {
        sendJson(response, 400, { error: 'human replay id must be a positive integer' });
        return;
      }
      ...
      if (typeof row.replayData !== 'string' || row.replayData.length === 0) {
        sendJson(response, 404, { error: 'Replay data is not available for this entry' });
        return;
      }
    ...
    if (kind === 'ai') {
      if (!id) {
        sendJson(response, 400, { error: 'ai replay id is required' });
        return;
      }
    ...
    sendJson(response, 400, { error: 'kind must be either human or ai' });
    return;
  }
  ...
  if (url.pathname === '/api/submit' && method === 'POST') {
    ...
    try {
      ...
      sendJson(response, 500, { error: 'Database write failed' });
      return;
    }
