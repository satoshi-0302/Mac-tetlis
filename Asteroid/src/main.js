    ...
    <aside class="side-column">
      ...
      <section class="panel result-panel hidden" id="resultPanel">
        ...
        <form id="submitForm" class="submit-form">
          <label>
            Name (max 5)
            <input id="nameInput" name="name" maxlength="5" required autocomplete="nickname" placeholder="ACE" />
          </label>
          <label>
            Message (max 30)
            <input id="messageInput" name="message" maxlength="30" autocomplete="off" placeholder="clean run" />
          </label>
...
function sanitizeName(value) {
  return value.replace(/[^\x20-\x7E]/g, '').trim().toUpperCase().slice(0, 5);
}

function sanitizeMessage(value) {
  return value.replace(/[^\x20-\x7E]/g, '').trim().slice(0, 30);
}
...
function refreshLeaderboardStatus(overrideText = '') {
  ...
  if (leaderboardSnapshot.aiEntries?.length) {
    leaderboardStatus.textContent = '1位は AI の現行ベストです。WATCH で replay を見られます。2位から10位は目標スコアの空欄です。';
    return;
  }

  leaderboardStatus.textContent = 'AI replay がまだ生成されていません。Refresh を押して再取得できます。';
}
...
function buildUnifiedLeaderboardEntries(snapshot) {
  ...
  for (let rank = 2; rank <= 10; rank += 1) {
    entries.push({
      id: `rank-${String(rank).padStart(2, '0')}`,
      rank,
      kind: '',
      name: '',
      message: '',
      score: 12_000 - rank * 1_000,
      replayAvailable: false,
      isPlaceholder: true
    });
  }
